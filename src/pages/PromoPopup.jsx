...
export default function PromoPopup({ restaurantId, restaurantSettings }) {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(ref(realtimeDB, `restaurants/${restaurantId}/promo`), (snap) => {
      ...
      // ✅ FIX 2: validTill expiry check — expire ho gayi date to popup mat dikhao
      if (data.validTill) {
        const expiry = new Date(data.validTill);
        expiry.setHours(23, 59, 59, 999); // us din ka aakhri second
        if (expiry < new Date()) return;   // expired — band karo
      }
