...
const navItems = [
  { to: "bookingtable",        label: "Table Booking",       icon: CalendarCheck   },
  { to: "adminorder",          label: "Admin Order",         icon: ClipboardList   },
  ...
  { to: "feedback-admin",      label: "Home Feedback",       icon: FileText        },
  { to: "delivery-management", label: "Delivery Boys",       icon: Bike            },
  { to: "inventory",           label: "Inventory",           icon: Package,        comingSoon: true },
  { to: "subscription",        label: "Subscription",        icon: CreditCard      },
  { to: "payment-status",      label: "Payment Status",      icon: Wallet          },
  { to: "multi-branch",        label: "Multi-Branch",        icon: GitBranch       },
  { to: "revenue",             label: "Revenue Dashboard",   icon: BarChart2       },
  { to: "kitchen-display",     label: "Kitchen Display",     icon: ChefHat         },
  { to: "admin-coupen",        label: "Admin Coupons",       icon: Ticket          },
];
  ...
  return (
    <>
      <style>{`
        ...
        .sb-title    { font-size: 17px; font-weight: 700; color: #fff; letter-spacing: 0.2px; }
        .sb-subtitle { font-size: 10px; color: rgba(255,255,255,0.5); letter-spacing: 1.5px; text-transform: uppercase; margin-top: 2px; }
        ...
        .sb-link:hover { background: rgba(255,255,255,0.1); color: #fff; transform: translateX(3px); }
        .sb-link.active {
        ...
        .sb-icon { display: flex; align-items: center; justify-content: center; width: 20px; flex-shrink: 0; }
          ...
          {navItems.map(({ to, label, icon: Icon, comingSoon }) =>
            comingSoon ? (
              <div key={to} className="sb-link" style={{ opacity: 0.5, cursor: "not-allowed" }}>
                ...
                <span style={{
                  fontSize: "9px", background: "#FFD166", color: "#6B1535",
