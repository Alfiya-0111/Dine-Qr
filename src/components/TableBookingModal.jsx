...
import { Calendar, Clock, Users, MapPin, XCircle, CheckCircle, Phone, Lock, Crown } from "lucide-react";

// ─── PLAN CONFIG (ONLY Trial & Pro have table booking) ──────────────────────
const PLAN_FEATURES = {
  trial: {
    tableBooking: true,
    maxTablesPerDay: 10,
  ...
  growth: {
    tableBooking: false,   // ❌ CHANGED: Growth mein ab nahi hai
    maxTablesPerDay: 0,
    maxGuests: 0,
    name: "Growth",
  },
...
const TableBookingModal = ({ isOpen, onClose, restaurantId, theme, userId }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(bookingsRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const todayCount = Object.values(bookings).filter(
          (b) => b.date === today && b.status !== "cancelled"
        ).length;
        setTodaysBookingCount(todayCount);
      } else {
  ...
  useEffect(() => {
    ...
    const unsubTables = onValue(tablesRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        tablesList = Object.entries(tablesData).map(([id, data]) => ({
          id,
          ...data,
  ...
  const canBookTable = () => {
    if (!planFeatures) return { allowed: false, reason: "Loading subscription...", needsUpgrade: false };
    if (!planFeatures.tableBooking) {
      return { allowed: false, reason: "Table booking is only available in Pro plan", needsUpgrade: true };
    }
    if (todaysBookingCount >= planFeatures.maxTablesPerDay) {
      return { allowed: false, reason: `Daily limit reached (${planFeatures.maxTablesPerDay} bookings/day)`, needsUpgrade: true };
    }
    if (guestCount > planFeatures.maxGuests) {
      return { allowed: false, reason: `Max ${planFeatures.maxGuests} guests allowed in your plan`, needsUpgrade: true };
    }
  ...
  const handleBooking = async () => {
    ...
    if (!check.allowed) {
      if (check.needsUpgrade) {
        ...
        // Sirf Pro plan suggest karo (Trial ke alawa only Pro hai)
        if (!planFeatures || !planFeatures.tableBooking) {
          setUpgradePlan({ id: "pro", name: "Pro", price: 999, features: PLAN_FEATURES.pro });
        } else if (guestCount > (planFeatures?.maxGuests || 0)) {
          setUpgradePlan({ id: "pro", name: "Pro", price: 999, features: PLAN_FEATURES.pro });
        } else {
          setUpgradePlan({ id: "pro", name: "Pro", price: 999, features: PLAN_FEATURES.pro });
        }
    ...
    try {
      ...
      await push(rtdbRef(realtimeDB, `notifications/${userId}`), {
        title: "🪑 Table Booked!",
        message: `Table ${selectedTable.name} booked for ${selectedDate} at ${selectedTime}`,
        type: "booking",
        createdAt: Date.now(),
        read: false,
      });
  ...
  // ─── RENDER UPGRADE MODAL (Only Pro suggested) ─────────────────────────────
    ...
    return (
      <>
        <style>{`
          ...
          .tbm-upgrade-plan-price {
            font-size: 36px;
            font-weight: 900;
            color: #111;
          }

          .tbm-upgrade-plan-price span {
            font-size: 14px;
            color: #888;
            font-weight: 400;
          }

          .tbm-upgrade-features {
            text-align: left;
            margin-bottom: 20px;
          }
          ...
          .tbm-upgrade-feature:last-child {
            border-bottom: none;
          }

          .tbm-upgrade-feature-check {
            color: #22c55e;
            font-weight: 700;
            font-size: 16px;
          }
        ...
        <div className="tbm-overlay" onClick={(e) => e.target === e.currentTarget && handleClose()}>
          <div className="tbm-upgrade-modal">
            <div className="tbm-upgrade-header">
              <div className="tbm-upgrade-icon">
            ...
            <div className="tbm-upgrade-body">
              <div className="tbm-upgrade-plan">
                <p className="tbm-upgrade-plan-name">{upgradePlan.name} Plan</p>
                <p className="tbm-upgrade-plan-price">
                  ₹{upgradePlan.price}<span>/month</span>
                </p>
              </div>
              <div className="tbm-upgrade-features">
                <div className="tbm-upgrade-feature">
                  <span className="tbm-upgrade-feature-check">✓</span>
                  <span>Table Booking Enabled</span>
                </div>
                <div className="tbm-upgrade-feature">
                  <span className="tbm-upgrade-feature-check">✓</span>
                  <span>Unlimited bookings/day</span>
                </div>
                <div className="tbm-upgrade-feature">
                  <span className="tbm-upgrade-feature-check">✓</span>
                  <span>Up to {upgradePlan.features.maxGuests} guests per booking</span>
                </div>
                <div className="tbm-upgrade-feature">
                  <span className="tbm-upgrade-feature-check">✓</span>
                  <span>WhatsApp Order Integration</span>
                </div>
                <div className="tbm-upgrade-feature">
                  <span className="tbm-upgrade-feature-check">✓</span>
                ...
                <div className="tbm-upgrade-feature">
                  <span className="tbm-upgrade-feature-check">✓</span>
                  <span>Delivery Boys Management</span>
                </div>
                <div className="tbm-upgrade-feature">
                  <span className="tbm-upgrade-feature-check">✓</span>
              ...
              <button 
                className="tbm-upgrade-btn" 
                onClick={() => window.location.href = '/subscription'}
              >
                Upgrade to Pro →
              </button>
              <button className="tbm-upgrade-skip" onClick={handleClose}>
                Maybe Later
              </button>
            </div>
          </div>
  ...
  return (
    <>
      <style>{`
        ...
        .tbm-date-btn:hover:not(.selected):not(:disabled) {
          border-color: #d1d5db;
          background: #f9fafb;
        }
        ...
        .tbm-time-btn:hover:not(.selected):not(:disabled) {
          border-color: #d1d5db;
          background: #f9fafb;
        }
        ...
        .tbm-table-btn:hover:not(.selected):not(:disabled) {
          border-color: #d1d5db;
          background: #f9fafb;
        }
        ...
        .tbm-plan-badge.growth { background: #fef3c7; color: #92400e; }
      ...
      <div className="tbm-overlay" onClick={(e) => e.target === e.currentTarget && handleClose()}>
        <div className="tbm-modal">
          ...
          <div className="tbm-header">
            ...
            <button className="tbm-close-btn" onClick={handleClose} aria-label="Close">
              <XCircle size={22} />
            </button>
          </div>
          ...
          {!bookingConfirmed && planFeatures && (
            <div style={{ padding: "0 20px", marginTop: 10 }}>
              ...
              {planFeatures.tableBooking && (
                <div className="tbm-limit-info">
                  <Users size={14} />
                  {todaysBookingCount}/{planFeatures.maxTablesPerDay} bookings today · Max {planFeatures.maxGuests} guests
                </div>
              )}

              {!planFeatures.tableBooking && (
                <div 
                  className="tbm-limit-warning" 
                  onClick={() => setShowUpgradeModal(true)}
                >
                  <Lock size={14} />
                  Table booking is only available in Pro plan. Upgrade now!
                </div>
              )}

              {planFeatures.tableBooking && todaysBookingCount >= planFeatures.maxTablesPerDay && (
                <div 
                  className="tbm-limit-warning"
                  onClick={() => setShowUpgradeModal(true)}
                >
                  <Lock size={14} />
                  Daily booking limit reached. Upgrade to Pro for unlimited bookings.
                </div>
              )}
            </div>
          )}
          ...
          <div className="tbm-body">
            {bookingConfirmed ? (
              ...
              // ─── LOCKED STATE (Starter/Growth) ─────────────────────────────
              <div className="tbm-locked">
                <div className="tbm-locked-icon">
                ...
                <p style={{ fontSize: 14, color: '#6b7280', margin: '0 0 20px', lineHeight: 1.5 }}>
                  Table booking is only available in <strong>Pro plan</strong>.<br/>
                  Upgrade to Pro for unlimited table bookings.
                </p>
                <button 
                  onClick={() => setShowUpgradeModal(true)}
              ...
              <>
                  ...
                  <div className="tbm-date-scroll">
                    {getNext7Days().map((day) => (
                      <button
                        ...
                        disabled={!planFeatures?.tableBooking || todaysBookingCount >= planFeatures?.maxTablesPerDay}
                ...
                <div className="tbm-section">
                  ...
                  <div className="tbm-guest-counter">
                    <button
                      ...
                      disabled={!planFeatures?.tableBooking || guestCount <= 1}
                    >−</button>
                    <span className="tbm-guest-count">{guestCount}</span>
                    <button
                      className="tbm-guest-btn"
                      onClick={() => setGuestCount(Math.min(planFeatures?.maxGuests || 2, guestCount + 1))}
                      disabled={!planFeatures?.tableBooking || guestCount >= (planFeatures?.maxGuests || 2)}
                    >+</button>
                  </div>
                  {planFeatures?.tableBooking && guestCount >= planFeatures?.maxGuests && (
                    <p style={{ fontSize: 11, color: '#92400e', marginTop: 6 }}>
                      Max {planFeatures.maxGuests} guests allowed in your plan
                    </p>
                  )}
                </div>

                {/* Tables */}
                {selectedDate && selectedTime && planFeatures?.tableBooking && (
                  <div className="tbm-section">
                      ...
                      <div className="tbm-tables-grid">
                        {availableTables.map((table) => (
                          <button
                            ...
                            disabled={todaysBookingCount >= planFeatures.maxTablesPerDay}
                ...
                <button
                  onClick={() => setStep(2)}
                  disabled={!selectedDate || !selectedTime || !selectedTable || !planFeatures?.tableBooking || todaysBookingCount >= planFeatures?.maxTablesPerDay}
                  className="tbm-btn-primary"
                >
                  Continue →
                </button>
